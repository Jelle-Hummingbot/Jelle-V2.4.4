...
def start(self):
    ...
    min_profitability = xemm_map.get(
    ...
    limit_order_min_expiration = xemm_map.get(
        "limit_order_min_expiration").value
    cancel_order_threshold = xemm_map.get(
        "cancel_order_threshold").value / Decimal("100")
    ...
    order_size_taker_volume_factor = xemm_map.get(
        "order_size_taker_volume_factor").value / Decimal("100")
    order_size_taker_balance_factor = xemm_map.get(
        "order_size_taker_balance_factor").value / Decimal("100")
    order_size_portfolio_ratio_limit = xemm_map.get(
        "order_size_portfolio_ratio_limit").value / Decimal("100")
    order_size_maker_balance_factor = xemm_map.get(
        "order_size_maker_balance_factor").value / Decimal("100")
    ...
    filled_order_delay_seconds = xemm_map.get(
        "filled_order_delay_seconds").value
    use_oracle_conversion_rate = xemm_map.get(
        "use_oracle_conversion_rate").value
    taker_to_maker_base_conversion_rate = xemm_map.get(
        "taker_to_maker_base_conversion_rate").value
    taker_to_maker_quote_conversion_rate = xemm_map.get(
        "taker_to_maker_quote_conversion_rate").value
    ...
    slippage_buffer_fix = xemm_map.get(
        "slippage_buffer_fix").value / Decimal("100")
    ...
    cancel_order_timer_seconds = xemm_map.get(
        "cancel_order_timer_seconds").value
    balance_fix_maker = xemm_map.get("balance_fix_maker").value
    use_min_profit_for_taker_price = xemm_map.get(
        "use_min_profit_for_taker_price").value
    maker_perpetual = xemm_map.get("maker_perpetual").value
    taker_perpetual = xemm_map.get("taker_perpetual").value
    ...
    if isinstance(top_depth_tolerance, list) or "trade_size_override" in xemm_map:
        self._notify(
            "Current config is not compatible with cross exchange market making strategy. Please reconfigure")
        return
    ...
    try:
        ...
        maker_assets: Tuple[str, str] = self._initialize_market_assets(
            maker_market, [maker_trading_pair])[0]
        taker_assets: Tuple[str, str] = self._initialize_market_assets(
            taker_market, [taker_trading_pair])[0]
        if triangular_switch:
            third_assets: Tuple[str, str] = self._initialize_market_assets(
                maker_market, [third_trading_pair])[0]
        else:
            third_assets: Tuple[str, str] = self._initialize_market_assets(
                taker_market, [third_trading_pair])[0]
    ...
    maker_data = [self.markets[maker_market],
                  maker_trading_pair] + list(maker_assets)
    taker_data = [self.markets[taker_market],
                  taker_trading_pair] + list(taker_assets)
    third_data = [self.markets[maker_market],
                  third_trading_pair] + list(third_assets)
    ...
    self.market_trading_pair_tuples = [maker_market_trading_pair_tuple,
                                       taker_market_trading_pair_tuple, third_market_trading_pair_tuple]
    ...
    self.strategy.init_params(
        ...
        use_min_profit_for_taker_price=use_min_profit_for_taker_price,
        maker_perpetual=maker_perpetual,
        taker_perpetual=taker_perpetual
    )
